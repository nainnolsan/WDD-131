...
document.getElementById("simple-sort").textContent = JSON.stringify(simpleSort, null, 2);

function searchList(list, query) {
  return list.filter(s => s.toLowerCase().includes(query.toLowerCase()));
}

document.getElementById("simple-filter").textContent = JSON.stringify(
  searchList(simpleList, "an"),
  null,
  2
);
...
document.getElementById("hikes-results").textContent = JSON.stringify(
  searchHikes(hikes, "al"),
  null,
  2
);
